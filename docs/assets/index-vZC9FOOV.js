var pe=Object.defineProperty;var he=(a,t,e)=>t in a?pe(a,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):a[t]=e;var o=(a,t,e)=>(he(a,typeof t!="symbol"?t+"":t,e),e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function e(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=e(s);fetch(s.href,i)}})();var W=(a=>(a.Default="default",a.Page="page",a.NotFound="not-found",a))(W||{});class se{constructor(t,e,n){o(this,"url");o(this,"type");o(this,"routing");this.url=t,this.type=e,this.routing=n}}class me extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"routes",[]);this.shadowRoot=this.attachShadow({mode:"closed"})}connectedCallback(){window.addEventListener("hashchange",()=>{this.checkRoute()})}addRoutes(e){this.routes=[...e],this.checkRoute()}checkRoute(){const e=window.location.hash.slice(2);this.changeRoute(e)}changeRoute(e){if(e){const n=this.routes.findIndex(s=>s.url===e);this.shadowRoot.innerHTML=this.routes[n]?this.routes[n].routing():this.sendNotFound()}else{const n=this.routes.filter(s=>s.type===W.Default);n?window.location.hash="#/"+n[0].url:this.sendNotFound()}}sendNotFound(){const e=this.routes.filter(n=>n.type===W.NotFound);return e.length===0||(window.location.hash="#/"+e[0].url,this.changeRoute(e[0].url)),"404: Not found"}}customElements.define("app-router",me);class K{constructor(t,e){o(this,"color");o(this,"opacity");this.color=t,this.opacity=e}static createEmpty(){return new K("#008000",1)}}class Z{constructor(t,e,n){o(this,"propertyName");o(this,"displayName");o(this,"type");this.propertyName=t,this.displayName=e,this.type=n}static createEmpty(){return new Z("","","string")}}var j=(a=>(a.String="string",a.Image="image",a.Number="number",a))(j||{});class z{constructor(t,e,n,s,i,r){o(this,"name");o(this,"layer");o(this,"url");o(this,"style");o(this,"tags");o(this,"relevantProperties");this.name=t,this.layer=e,this.url=n,this.style=s,this.tags=i,this.relevantProperties=r}static createEmpty(){return new z("","","",K.createEmpty(),[],[Z.createEmpty()])}}const v=class v{constructor(){o(this,"CATEGORIES_URL","./json/categories.json");o(this,"_data");if(v._instance)return v._instance;v._instance=this}static get instance(){return v._instance||(v._instance=new v),v._instance}get data(){return this._data}set data(t){this._data=t}async getData(){if(this.data)return this.data;{let t=await this.fetchAppData(this.CATEGORIES_URL);return t=this.parseData(t),this.data=t,t}}async fetchAppData(t){try{const e=await fetch(t).then(s=>s.json()),n=await Promise.all(e.categories.map(async s=>{const i=await Promise.all(s.groups.map(async r=>{if(typeof r=="string")try{const l=await fetch(r);if(l.ok)return l.json();throw new Error("Errore durante il recupero dei dati.")}catch(l){return console.error(l),null}else return r}));return s.groups=i,s}));return{...e,categories:n}}catch(e){throw console.error("Errore durante il recupero dei dati JSON.",e),e}}parseData(t){return{categories:t.categories.map(n=>({name:n.name,groups:n.groups.map(s=>this.parseGroup(s))}))}}parseGroup(t){return Array.isArray(t)?t:{name:t.name,layers:t.layers.map(e=>this.parseLayer(e))}}parseLayer(t){return new z(t.name,t.layer,t.layer_url_wfs,new K(t.style.color,parseFloat(t.style.opacity)),t.tags,t.relevant_properties.map(e=>{let n=Z.createEmpty();switch(n.displayName=e.display_name,n.propertyName=e.property_name,e.type){case"image":n.type=j.Image;break;case"number":n.type=j.Number;break;default:n.type=j.String;break}return n}))}getAllLayers(t){const e=[];return t.categories.map(n=>{n.groups.map(s=>{typeof s!="string"&&s.layers.map(i=>{e.push(i)})})}),e}filterLayersByNameAndTag(t,e){let n=[];return n=t.categories.flatMap(s=>s.groups.flatMap(i=>typeof i=="string"?[z.createEmpty()]:i.layers.filter(r=>r.name.toLowerCase().includes(e)||r.tags.some(l=>l.includes(e))))),n}getAllTags(t){let e=[];return t.categories.map(s=>{s.groups.map(i=>{typeof i!="string"&&i.layers.map(r=>{r.tags.map(l=>{e.push(l)})})})}),[...new Set(e)]}filterLayersByTag(t,e){let n=[];return n=t.categories.flatMap(s=>s.groups.flatMap(i=>typeof i=="string"?[z.createEmpty()]:i.layers.filter(r=>r.tags.some(l=>l.includes(e))))),n}filterLayersByTags(t,e){let n=[];return e.forEach(i=>{this.filterLayersByTag(t,i).forEach(l=>n.push(l))}),[...new Set(n)]}};o(v,"_instance");let _=v;const be=`.page {
    display: flex;
...
}`;class ge extends HTMLElement{constructor(){super();o(this,"shadowRoot");this.shadowRoot=this.attachShadow({mode:"closed"});const e=new CSSStyleSheet;e.replaceSync(be),this.shadowRoot.adoptedStyleSheets.push(e)}async connectedCallback(){await _.instance.getData(),this.render()}render(){this.shadowRoot.innerHTML=`
            <div class="page">
                <div class="box">
                    <app-tags-wall></app-tags-wall>
                </div>
            </div>
            `}}customElements.define("page-tags",ge);const k=class k{constructor(){o(this,"_position",null);if(k._instance)return k._instance;k._instance=this}get position(){return this._position}set position(t){this._position=t}static get instance(){return k._instance||(k._instance=new k),k._instance}async getUserPosition(){try{const t=await new Promise((e,n)=>{navigator.geolocation.getCurrentPosition(s=>{e(s)},s=>{n(s)})});this._position=t}catch{this._position=null}}static geolocationToCartographic(t){return new Cesium.Cartographic(t.coords.longitude,t.coords.latitude,t.coords.altitude||0)}};o(k,"_instance");let w=k;class ee{constructor(t,e,n,s,i,r){o(this,"uuid");o(this,"name");o(this,"position");o(this,"type");o(this,"layer");o(this,"props");this.uuid=t,this.name=e,this.position=n,this.type=s,this.layer=i,this.props=r}static createEmpty(){return new ee("","",Cesium.Cartographic.ZERO,"point",z.createEmpty(),[])}}class ne{constructor(t,e,n){o(this,"displayName");o(this,"type");o(this,"value");this.displayName=t,this.type=e,this.value=n}static createEmpty(){return new ne("",j.String,"")}}var V=(a=>(a.Point="point",a.Polyline="polyline",a.Polygon="polygon",a))(V||{});class ${constructor(t,e,n){o(this,"name");o(this,"pois");o(this,"lastSelected");this.name=t,this.pois=e,this.lastSelected=n}static createEmpty(){return new $("",[],!0)}static createDefault(){return new $("default",[],!0)}}var y=(a=>(a.Info="info",a.SuggestedPath="suggested-path",a.CustomPath="custom-path",a))(y||{});const P=class P{constructor(){o(this,"listeners",{});if(P._instance)return P._instance;P._instance=this}static get instance(){return P._instance||(P._instance=new P),P._instance}subscribe(t,e){this.listeners[t]||(this.listeners[t]=[]),this.listeners[t].push(e)}unsubscribe(t,e){this.listeners[t]&&(this.listeners[t]=this.listeners[t].filter(n=>n!==e))}unsubscribeAll(t){delete this.listeners[t]}publish(t,e){this.listeners[t]&&this.listeners[t].forEach(n=>n(e))}};o(P,"_instance");let c=P;const S=class S{constructor(){o(this,"_currentTab",y.Info);if(S._instance)return S._instance;S._instance=this}static get instance(){return S._instance||(S._instance=new S),S._instance}get currentTab(){return this._currentTab}set currentTab(t){this._currentTab=t,c.instance.publish("current-tab-updated",this.currentTab)}};o(S,"_instance");let Q=S;var b=(a=>(a.Loader="loader",a.Temporary="temporary",a.Error="error",a.Info="info",a))(b||{});const fe=`:host {
    position: relative;
...
@media screen and (max-width: 768px) {
    ...
    app-tabs-sidenav {
        ...
        left: 0;
...
}`;class ke extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"map",new oe);this.shadowRoot=this.attachShadow({mode:"closed"});let e=new CSSStyleSheet;e.replaceSync(ve),this.shadowRoot.adoptedStyleSheets.push(e)}async connectedCallback(){await _.instance.getData(),await J.instance.getMapThemes(),await w.instance.getUserPosition(),d.instance.paths.some(n=>n.name==="default")||d.instance.saveNewPath("default");const e=d.instance.paths.find(n=>n.lastSelected===!0);e&&(d.instance.selectedCustomPath={...e}),this.render(),this.setup()}render(){this.shadowRoot.innerHTML=`
            <div class="page">
                <app-map></app-map>
                <div class="header">
                    <div class="search">
            ...
            `,this.legend=this.querySelector(".legend"),this.legend.style.backgroundColor=h.rgbToRgba(h.hexToRgb(this._layer.style.color),.5),this.legend.style.borderColor=this._layer.style.color}setup(){this.addEventListener("click",()=>{c.instance.publish("add-layer",this.layer)})}}customElements.define("app-search-result-chip",re,{extends:"button"});const Ee=`:host {
    display: none;
}
...
`,Te=`button[is="app-search-result-chip"] {
    cursor: pointer;
    display: flex;
    align-items: center;
    justify-content: space-between;
...
button[is="app-search-result-chip"] .icon {
    font-size: 1.4rem;
}`;class Me extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"container",document.createElement("div"));o(this,"_layers",[]);o(this,"_isVisible",!1);this.shadowRoot=this.attachShadow({mode:"closed"});let e=new CSSStyleSheet;e.replaceSync(Ee);let n=new CSSStyleSheet;n.replaceSync(Te),this.shadowRoot.adoptedStyleSheets=[e,n]}get layers(){return this._layers}set layers(e){this._layers=e,this.update()}get isVisible(){return this._isVisible}set isVisible(e){this._isVisible=e,this._isVisible===!0?this.classList.add("visible"):this.classList.remove("visible")}connectedCallback(){this.render(),this.setup()}render(){this.container.classList.add("container"),this.shadowRoot.append(this.container)}setup(){c.instance.subscribe("search-layer",e=>{if(this.container.innerHTML="",e.searchValue===""){this.isVisible=!1;return}this.isVisible=!0,this.layers=e.layers})}update(){if(this._layers.length===0){let e=document.createElement("p");e.innerHTML="Nessun livello trovato",this.container.append(e);return}this._layers.forEach(e=>{let n=new re;n.layer=e,n.setAttribute("is","app-search-result-chip"),this.container.append(n)})}disconnectedCallback(){c.instance.unsubscribeAll("search-layer")}}customElements.define("app-search-result",Me);class Re extends HTMLButtonElement{constructor(){super();o(this,"_isOpen",!1);o(this,"icon");this.icon=this.querySelector(".material-symbols-outlined")}get isOpen(){return this._isOpen}set isOpen(e){this._isOpen=e,this.update()}connectedCallback(){this.setup()}setup(){this.addEventListener("click",()=>{X.instance.isOpen=!this.isOpen}),c.instance.subscribe("toggle-tabs",e=>{this.isOpen=e})}update(){this.isOpen?this.icon.innerHTML="close":this.icon.innerHTML="menu"}disconnectedCallback(){c.instance.unsubscribeAll("toggle-tabs")}}customElements.define("app-tabs-toggle",Re,{extends:"button"});const Ie=`:host {
    width: 360px;
    height: 100vh;
    background-color: var(--surface-container);
    animation: slideOut .3s ease-in-out forwards;
...
}`;class Ae extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"_isVisible",!1);this.shadowRoot=this.attachShadow({mode:"closed"});let e=new CSSStyleSheet;e.replaceSync(Ie),this.shadowRoot.adoptedStyleSheets.push(e)}get isVisible(){return this._isVisible}set isVisible(e){this._isVisible=e,this.update()}connectedCallback(){this.render(),this.setup()}render(){this.shadowRoot.innerHTML=`
            <div class="toggle">
                <div class="close"></div>
            </div>
            <app-tabs></app-tabs>
...
}`,Oe=`button[is="app-carousel-chip"] {
    cursor: pointer;
    display: flex;
    align-items: center;
    justify-content: space-between;
...
button[is="app-carousel-chip"] .icon {
    ...
    &:hover {
        background-color: var(--surface-container-lowest);
            ...
            `,this.info=this.querySelector(".info"),this.legend=this.querySelector(".legend"),this.legend.style.backgroundColor=h.rgbToRgba(h.hexToRgb(this._layer.style.color),.5),this.legend.style.borderColor=this._layer.style.color,this.removeIcon=this.querySelector(".icon")}setup(){this.addEventListener("click",()=>{c.instance.publish("unbench-layer",this.layer)}),this.removeIcon.addEventListener("click",e=>{e.stopPropagation(),c.instance.publish("remove-layer-from-bench",this.layer)})}}customElements.define("app-bench-chip",ce,{extends:"button"});const _e=`:host {
    width: 360px;
    height: fit-content;
...
}`;class Ve extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"_currentTab",y.Info);o(this,"infoTab",null);o(this,"suggestedRouteTab",null);o(this,"customRouteTab",null);o(this,"panel",null);this.shadowRoot=this.attachShadow({mode:"closed"});let e=new CSSStyleSheet;e.replaceSync(Fe),this.shadowRoot.adoptedStyleSheets.push(e)}get currentTab(){return this._currentTab}set currentTab(e){this._currentTab=e,this.switchPanel(),this.switchTab()}connectedCallback(){this.render(),this.setup(),this.currentTab=y.Info}render(){this.shadowRoot.innerHTML=`
            <nav class="header">
...
.property-value {
    color: var(--on-surface);
}`;class qe extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"_poi",O.instance.selectedPoi);o(this,"_isInfoOpen",!1);this.shadowRoot=this.attachShadow({mode:"closed"});let e=new CSSStyleSheet;e.replaceSync(je),this.shadowRoot.adoptedStyleSheets.push(e)}get poi(){return this._poi}set poi(e){this._poi=e,this.isInfoOpen=!1,this.update()}get isInfoOpen(){return this._isInfoOpen}set isInfoOpen(e){this._isInfoOpen=e,this.toggleInfo()}connectedCallback(){this.render(),this.setup(),this.poi&&this.update()}disconnectedCallback(){c.instance.unsubscribe("selected-poi",this.handleSelectedPoi.bind(this))}handleSelectedPoi(e){this.poi=e}render(){this.shadowRoot.innerHTML='<p class="empty-msg">Nessun punto selezionato</p>'}setup(){this.handleSelectedPoi=this.handleSelectedPoi.bind(this),c.instance.subscribe("selected-poi",this.handleSelectedPoi)}update(){if(!this.poi){this.render();return}this.shadowRoot.innerHTML=`
            <div class="header">
                <div class="title">
                    <span class="legend"></span>
            ...
            <div class="tools"></div>
            `;const e=this.shadowRoot.querySelector(".legend"),n=this.shadowRoot.querySelector(".category"),s=this.shadowRoot.querySelector(".tools");e.style.backgroundColor=h.rgbToRgba(h.hexToRgb(this._poi.layer.style.color),.5),e.style.borderColor=this._poi.layer.style.color,this.poi.props.forEach(u=>{u.displayName==="Nome"?n.innerHTML=u.value:n.innerHTML=this.poi.name});const i=this.renderDirectionsBtn();i&&s.appendChild(i);const r=this.renderAddToRouteBtn();r&&s.append(r);const l=this.renderInfo();l&&this.shadowRoot.appendChild(l)}renderDirectionsBtn(){if(!this.poi)return null;const e=document.createElement("button");return e.classList.add("directions-btn"),e.innerHTML="Indicazioni",e.addEventListener("click",()=>B.instance.openGoogleMaps(this.poi.position)),e}renderAddToRouteBtn(){if(!this.poi||this.poi.type!==V.Point)return null;const e=document.createElement("button");return e.classList.add("add-to-path-btn"),e.innerHTML="Aggiungi",e.addEventListener("click",()=>{this.poi&&d.instance.addPoiToSelectedPath(this.poi)}),e}renderInfo(){if(!this.poi)return null;const e=this.poi.props.filter(r=>r.displayName!=="Nome");if(e.length===0)return null;let n=document.createElement("div");n.classList.add("info");const s=this.renderMoreInfoBtn();n.appendChild(s);const i=document.createElement("div");return i.classList.add("info-content"),n.appendChild(i),e.forEach(r=>{const l=this.renderTopic(r);i.appendChild(l)}),n}renderMoreInfoBtn(){const e=document.createElement("button");return e.innerHTML="Leggi info",e.classList.add("toggle-info"),e.addEventListener("click",()=>this.isInfoOpen=!this.isInfoOpen),e}toggleInfo(){const e=this.shadowRoot.querySelector(".info-content"),n=this.shadowRoot.querySelector(".toggle-info");e&&n&&(this.isInfoOpen?e.classList.add("visible"):e.classList.remove("visible"),this.isInfoOpen?n.innerHTML="Mostra meno":n.innerHTML="Leggi info")}renderTopic(e){const n=document.createElement("div");n.classList.add("property");const s=document.createElement("label");s.classList.add("property-label"),s.innerHTML=e.displayName;const i=document.createElement("p");return i.classList.add("property-value"),e.value!==""?i.innerHTML=e.value:i.innerHTML="-",n.appendChild(s),n.appendChild(i),n}}customElements.define("app-info-panel",qe);var D=(a=>(a.SortPois="sort-pois",a.EditPath="edit-path",a.AddPath="add-path",a.BookmarkPath="bookmark-path",a.LoadPath="load-path",a))(D||{});const F=class F{constructor(){F._instance||(F._instance=this)}static get instance(){return F._instance||(F._instance=new F),F._instance}calculateDistance(t,e){const n=t.longitude-e.longitude,s=t.latitude-e.latitude;return Math.sqrt(n*n+s*s)}nearestInsertion(t,e){const n=[...t];let s=0,i=this.calculateDistance(e,n[0].position);for(let l=1;l<n.length;l++){const u=this.calculateDistance(e,n[l].position);u<i&&(i=u,s=l)}const r=[n.splice(s,1)[0]];for(;n.length>0;){i=Number.MAX_VALUE;let l=0;for(let u=0;u<n.length;u++){const p=this.calculateDistance(r[r.length-1].position,n[u].position);p<i&&(i=p,l=u)}r.push(n.splice(l,1)[0])}return r.reverse()}};o(F,"_instance");let te=F;const Ge=`.form {
    color: var(--on-surface);
}
...
button.delete-btn {
    ...
    border-color: var(--error-container);
            ...
            `}setupSortPoisForm(){const e=this.shadowRoot.querySelector(".form");e&&e.addEventListener("submit",()=>{const n=w.instance.position;if(!n)return;const s=w.geolocationToCartographic(n),i=te.instance.nearestInsertion(d.instance.selectedCustomPath.pois,s),r=d.instance.selectedCustomPath;r.pois=i,d.instance.selectedCustomPath=r})}renderEditPathForm(){this.shadowRoot.innerHTML=`
            <form class="form">
            ...
            `}setupEditPathForm(){const e=this.shadowRoot.querySelector("input");if(!e)return;const n=this.shadowRoot.querySelector(".delete-btn");if(!n)return;const s=this.shadowRoot.querySelector(".submit-btn");if(!s)return;const i=this.shadowRoot.querySelector(".form");if(!i)return;e.value=d.instance.selectedCustomPath.name;const r=()=>s.disabled=e.value.trim().length===0||d.instance.paths.some(l=>l.name===e.value.toLowerCase());e.addEventListener("input",r),e.addEventListener("change",r),i.addEventListener("submit",()=>{const u=new FormData(i).get("path-name");u&&d.instance.editPath(u.toString())}),n.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("close-dialog")),d.instance.deletePath()})}renderAddPathForm(){this.shadowRoot.innerHTML=`
            <form class="form">
            ...
            `}setupAddPathForm(){const e=this.shadowRoot.querySelector("input");if(!e)return;const n=this.shadowRoot.querySelector(".submit-btn");if(!n)return;const s=this.shadowRoot.querySelector(".form");if(!s)return;e.value.length===0&&(n.disabled=!0);const i=()=>{n.disabled=e.value.trim().length===0||d.instance.paths.some(r=>r.name===e.value.toLowerCase())};e.addEventListener("input",i),e.addEventListener("change",i),s.addEventListener("submit",()=>d.instance.saveNewPath(e.value))}renderBookmarkPathForm(){var e;this.shadowRoot.innerHTML=`
            <form class="form">
...
.material-symbols-outlined {
    ...
    font-variation-settings:
        ...
        'opsz' 24;
    font-size: 1.5rem;
}`;class ue extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"_poi",null);this.shadowRoot=this.attachShadow({mode:"closed"});let e=new CSSStyleSheet;e.replaceSync($e),this.shadowRoot.adoptedStyleSheets.push(e)}get poi(){return this._poi}set poi(e){this._poi=e}connectedCallback(){this.render(),this.update(),this.setup()}render(){this.shadowRoot.innerHTML=`
            ...
            `}update(){if(!this.poi)return;const e=this.shadowRoot.querySelector(".order");e&&(e.innerHTML=(d.instance.selectedCustomPath.pois.indexOf(this.poi)+1).toString());const n=this.shadowRoot.querySelector(".name");n&&(n.innerHTML=this.poi.name);const s=this.shadowRoot.querySelector(".legend");s&&(s.style.backgroundColor=h.rgbToRgba(h.hexToRgb(this.poi.layer.style.color),.5),s.style.borderColor=this.poi.layer.style.color);const i=this.shadowRoot.querySelector(".category");i&&this.poi.props.forEach(r=>{r.displayName==="Nome"?i.innerHTML=r.value:i.innerHTML=this.poi.name})}setup(){this.poi&&(this.addEventListener("click",()=>O.instance.selectedPoi=this.poi),this.setupOrderBtns(),this.setupRemoveBtn())}setupOrderBtns(){const e=this.shadowRoot.querySelector(".move-up");e&&e.addEventListener("click",s=>{s.stopPropagation(),this.changeOrder("up")});const n=this.shadowRoot.querySelector(".move-down");n&&n.addEventListener("click",s=>{s.stopPropagation(),this.changeOrder("down")})}changeOrder(e){if(!this.poi)return;let n=d.instance.selectedCustomPath,s=[...n.pois],i=d.instance.selectedCustomPath.pois.indexOf(this.poi);s.splice(i,1),e==="up"?s.splice(i-1,0,this.poi):s.splice(i+1,0,this.poi),n.pois=s,d.instance.selectedCustomPath=n}setupRemoveBtn(){if(!this.poi)return;const e=this.shadowRoot.querySelector(".remove-btn");e&&e.addEventListener("click",n=>{n.stopPropagation();let s=d.instance.selectedCustomPath.pois.indexOf(this.poi),i=[...d.instance.selectedCustomPath.pois];i.splice(s,1),d.instance.selectedCustomPath={...d.instance.selectedCustomPath,pois:i}})}}customElements.define("app-custom-path-card",ue);const Je=`:host {
    display: flex;
    flex-direction: column;
    justify-content: space-between;
    height: 100%;
...
.material-symbols-outlined {
    ...
    font-variation-settings:
        ...
        'GRAD' 0,
        'opsz' 24;
}`;class Ue extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"_path",{...d.instance.selectedCustomPath});this.shadowRoot=this.attachShadow({mode:"closed"});let e=new CSSStyleSheet;e.replaceSync(Je),this.shadowRoot.adoptedStyleSheets.push(e)}get path(){return this._path}set path(e){this._path=e,c.instance.publish("load-custom-path",this.path)}connectedCallback(){this.render(),this.setup(),this.update()}render(){this.shadowRoot.innerHTML=`
            ...
            `}setup(){const e=this.shadowRoot.querySelector(".sort-btn"),n=this.shadowRoot.querySelector(".edit-btn"),s=this.shadowRoot.querySelector(".add-btn"),i=this.shadowRoot.querySelector(".bookmark-btn"),r=this.shadowRoot.querySelector(".load-btn");e&&e.addEventListener("click",()=>G.instance.createFormDialog(D.SortPois)),n&&n.addEventListener("click",()=>G.instance.createFormDialog(D.EditPath)),s&&s.addEventListener("click",()=>G.instance.createFormDialog(D.AddPath)),i&&i.addEventListener("click",()=>G.instance.createFormDialog(D.BookmarkPath)),r&&r.addEventListener("click",()=>G.instance.createFormDialog(D.LoadPath)),c.instance.subscribe("selected-custom-path-updated",l=>{this.path=l})}update(){const e=this.shadowRoot.querySelector(".list");if(!e)return;e.innerHTML="",this.path.pois.forEach(s=>{let i=new ue;i.poi=s,e.appendChild(i)});const n=this.shadowRoot.querySelector(".edit-btn");n&&this.path.name==="default"&&(n.disabled=!0)}disconnectedCallback(){c.instance.unsubscribeAll("selected-custom-path-updated")}}customElements.define("app-custom-path-panel",Ue);class We extends HTMLButtonElement{constructor(){super();o(this,"_isOpen",!1)}get isOpen(){return this._isOpen}set isOpen(e){this._isOpen=e,this.isOpen===!0?this.classList.add("open"):this.classList.remove("open")}connectedCallback(){this.setup()}setup(){this.addEventListener("click",()=>J.instance.togglePhysicalMap()),c.instance.subscribe("toggle-tabs",e=>{this.isOpen=e})}disconnectedCallback(){c.instance.unsubscribeAll("toggle-tabs")}}customElements.define("app-map-type-btn",We,{extends:"button"});const Xe=`:host {
    position: fixed;
    bottom: 24px;
    right: 24px;
    display: flex;
...
@media screen and (max-width: 768px) {
    ...
    @keyframes slideOut {
        ...
        to {
...
}`;class Ke extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"_isOpen",!1);this.shadowRoot=this.attachShadow({mode:"closed"});let e=new CSSStyleSheet;e.replaceSync(Xe),this.shadowRoot.adoptedStyleSheets.push(e)}get isOpen(){return this._isOpen}set isOpen(e){this._isOpen=e,this.isOpen===!0?this.classList.add("open"):this.classList.remove("open")}connectedCallback(){this.render(),this.setup()}render(){this.shadowRoot.innerHTML=`
            ...
            `}setup(){c.instance.subscribe("toggle-tabs",e=>this.isOpen=e)}disconnectedCallback(){c.instance.unsubscribeAll("toggle-tabs")}}customElements.define("app-map-controls",Ke);const Ye=`.header {
    text-align: center;
}
...
button.submit-btn {
    color: var(--on-primary);
    background-color: var(--primary);
}`;class Ze extends HTMLElement{constructor(){super();o(this,"shadowRoot");o(this,"_tags",[]);this.shadowRoot=this.attachShadow({mode:"closed"});const e=new CSSStyleSheet;e.replaceSync(Ye),this.shadowRoot.adoptedStyleSheets.push(e)}get tags(){return this._tags}set tags(e){this._tags=e}connectedCallback(){this.tags=_.instance.getAllTags(_.instance.data),this.render(),this.setup()}render(){this.shadowRoot.innerHTML=`          
            <div class="header">
            ...
            `;const e=this.shadowRoot.querySelector(".tags-wall");e&&this.tags.forEach(n=>{let s=this.createChip(n);e.append(s)})}setup(){const e=this.shadowRoot.querySelector("form");e&&e.addEventListener("submit",n=>{n.preventDefault();const s=new FormData(e),i=Array.from(s.getAll("tag"),l=>String(l));d.instance.setTags(i);const r=_.instance.filterLayersByTags(_.instance.data,i);d.instance.activeLayers=r,d.instance.benchLayers=[],window.location.hash="/map"})}createChip(e){let n=document.createElement("div");n.classList.add("chip");let s=document.createElement("input");s.type="checkbox",s.id=e.replace(" ","").toLowerCase(),s.name="tag",s.value=e,d.instance.tags.forEach(r=>{r===e&&(s.checked=!0)});let i=document.createElement("label");return i.setAttribute("for",e.replace(" ","").toLowerCase()),i.innerHTML=e.charAt(0).toUpperCase()+e.slice(1),n.append(s),n.append(i),n}}customElements.define("app-tags-wall",Ze);const Qe=document.querySelector("app-router"),en=new se("map",W.Page,()=>"<page-map></page-map>"),nn=new se("index",W.Default,()=>"<page-tags></page-tags>"),tn=new se("404",W.NotFound,()=>"<div>404</div>"),sn=[en,nn,tn];Qe.addRoutes(sn);d.instance.getTags();d.instance.getSavedLayers();d.instance.getCustomPaths();
