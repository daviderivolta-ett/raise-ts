...
.tool-icon {\r
    font-size: 1rem;\r
            ...
            `}setup(){const e=this.shadowRoot.querySelector(".sort-btn"),n=this.shadowRoot.querySelector(".edit-btn"),r=this.shadowRoot.querySelector(".add-btn"),s=this.shadowRoot.querySelector(".bookmark-btn"),c=this.shadowRoot.querySelector(".load-btn"),p=this.shadowRoot.querySelector('button[is="app-custom-path-download-btn"]');e&&e.addEventListener("click",()=>ge.instance.createFormDialog(ie.SortPois)),n&&n.addEventListener("click",()=>ge.instance.createFormDialog(ie.EditPath)),r&&r.addEventListener("click",()=>ge.instance.createFormDialog(ie.AddPath)),s&&s.addEventListener("click",()=>ge.instance.createFormDialog(ie.BookmarkPath)),c&&c.addEventListener("click",()=>ge.instance.createFormDialog(ie.LoadPath)),p&&(p.path={...this.path}),l.instance.subscribe("selected-custom-path-updated",y=>{this.path=y})}update(){const e=this.shadowRoot.querySelector(".list");if(!e)return;e.innerHTML="",this.path.pois.forEach(r=>{let s=new Cn;s.poi=r,e.appendChild(s)});const n=this.shadowRoot.querySelector(".edit-btn");n&&this.path.name==="default"&&(n.disabled=!0)}disconnectedCallback(){l.instance.unsubscribeAll("selected-custom-path-updated")}}customElements.define("app-custom-path-panel",Mt);class Rt extends HTMLButtonElement{constructor(){super();i(this,"_isOpen",!1)}get isOpen(){return this._isOpen}set isOpen(e){this._isOpen=e,this.isOpen===!0?this.classList.add("open"):this.classList.remove("open")}connectedCallback(){this.setup()}setup(){this.addEventListener("click",()=>ae.instance.togglePhysicalMap()),l.instance.subscribe("toggle-tabs",e=>{this.isOpen=e})}disconnectedCallback(){l.instance.unsubscribeAll("toggle-tabs")}}customElements.define("app-map-type-btn",Rt,{extends:"button"});const It=`:host {\r
    position: fixed;\r
    bottom: 24px;\r
    right: 24px;\r
    display: flex;\r
...
.category {
    ...
    padding: 0 0 0 22px;
            ...
            <div class="list"></div>
            `}setup(){const e=this.shadowRoot.querySelector(".back-btn");e&&e.addEventListener("click",()=>{ue.instance.isSuggestedPathSelected=!1,ue.instance.currentTab=E.SuggestedPath});const n=this.shadowRoot.querySelector(".load-layers-btn");n&&n.addEventListener("click",()=>{const r=this.getLayersInPath();l.instance.publish("bench-all-layers",null),r.forEach(s=>l.instance.publish("add-layer",s))})}update(){const e=this.shadowRoot.querySelector(".list");e&&(e.innerHTML="",this.path.pois.forEach(n=>{let r=new Tn;r.poi=n,e.appendChild(r)}))}getLayersInPath(){let e=[];return this.path.pois.forEach(n=>e.push(n.layer)),[...new Set(e)]}}customElements.define("app-selected-suggested-path-panel",Vt);const qt=document.querySelector("app-router"),$t=new We("map",ve.Page,()=>"<page-map></page-map>"),Gt=new We("index",ve.Default,()=>"<page-tags></page-tags>"),Ut=new We("404",ve.NotFound,()=>"<div>404</div>"),Wt=[$t,Gt,Ut];qt.addRoutes(Wt);ae.instance.getMapThemes().then(()=>ae.instance.getPreferColorScheme());h.instance.getTags();h.instance.getCsvPaths(1);h.instance.getSavedLayers();h.instance.getCustomPaths();
